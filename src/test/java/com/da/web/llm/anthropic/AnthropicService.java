...
public class AnthropicService implements Handler {
    ...
    private com.da.web.llm.provider.LlmProvider llmProvider;
    ...
    private ChatCompletionResponse buildResponse(ChatCompletionRequest request, String reply) {
        ...
        com.da.web.llm.anthropic.ChatCompletionResponse.Content content = new com.da.web.llm.anthropic.ChatCompletionResponse.Content();
        content.setType("text");
        content.setText(reply);
        response.setContent(new com.da.web.llm.anthropic.ChatCompletionResponse.Content[]{content});
    ...
    private String responseToJson(ChatCompletionResponse response) {
        ...
        if (response.getContent() != null && response.getContent().length > 0) {
            for (int i = 0; i < response.getContent().length; i++) {
                if (i > 0) sb.append(",");
                com.da.web.llm.anthropic.ChatCompletionResponse.Content content = response.getContent()[i];
                sb.append("{");
